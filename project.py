...
@FlowProject.label
def fv_done(job):
    return (job.document.get('free_volumes') != None)
...
def compute_fv(job):
	...
	# Compute any properties that are specific to each depletion
        #radius_ratios = [0.03, 0.04, 0.05, 0.047140452079103182, 0.06, 0.07, 0.08, 0.09, 0.1]
        radius_ratio = 0.05

        # Get existing free volumes if they are there.
        try:
            avg_free_vol = job.document['free_volume']
        except KeyError:
            avg_free_vol = {}
